...
def fairmult(sensitive: GraphSignal, exclude: GraphSignal = None):
    ...
    def method(original: GraphSignal, prule: float = 1):
        ...
        ret = sensitive*original*prule + (1-sensitive)*original*(mean_sensitive/mean_non_sensitive)
...
def lfpro(sensitive: GraphSignal, exclude: GraphSignal = None):
    def distribute(DR: GraphSignal, ranks: GraphSignal, eps: float):
        min_rank = float('inf')
    ...
    @lazy_no_cache
    @autoaspects
    def method(ranks: GraphSignal, eps=1.E-12):
        ...
        if sumR < phi:
            red = distribute(phi - sumR, ranks, {v: 1 - sensitive.get(v, 0) for v in ranks}, eps)
            ranks = {v: red.get(v, ranks[v] + (phi - sumR) / numR) for v in ranks}
        elif sumB < 1-phi:
            red = distribute(1-phi - sumB, ranks, {v: sensitive.get(v, 0) for v in ranks}, eps)
