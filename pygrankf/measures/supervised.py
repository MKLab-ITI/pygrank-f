...
@eager_no_cache
def mmabs(test: GraphSignal, prediction: GraphSignal, exclude: Optional[GraphSignal] = None) -> float:
    assert test is not None
    prediction = prediction.filter(exclude)
    test = test.filter(exclude)
...
@eager_no_cache
def cos(test: GraphSignal, prediction: GraphSignal, exclude: Optional[GraphSignal] = None) -> float:
    assert test is not None
    prediction = prediction.filter(exclude)
    test = test.filter(exclude)
...
def sgn(x):
    return x/(backend.abs(x)+1.E-12)


@eager_no_cache
def nce(test: GraphSignal, prediction: GraphSignal, exclude: Optional[GraphSignal] = None) -> float:
    assert test is not None
    prediction = prediction.filter(exclude)
    prediction = prediction / backend.sum(prediction)
    test = test.filter(exclude)
    test = test/backend.sum(test)
    return backend.sum(backend.log(prediction+1.E-12)*test)/backend.sum(backend.log(test+1.E-12)*test)
...
@eager_no_cache
def spearman(test: GraphSignal, prediction: GraphSignal, exclude: Optional[GraphSignal] = None) -> float:
    assert test and prediction
    prediction = np.array(prediction.filter(exclude), copy=True)
    test = np.array(test.filter(exclude), copy=True)
...
@eager_no_cache
def pearson(test: GraphSignal, prediction: GraphSignal, exclude: Optional[GraphSignal] = None) -> float:
    assert test and prediction
    prediction = prediction.filter(exclude)
    test = test.filter(exclude)
    cov = backend.length(test)*backend.sum(test*prediction)-backend.sum(prediction)*backend.sum(test)
    var_test = backend.length(test)*backend.sum(test**2) - backend.sum(test)**2
    var_pred = backend.length(prediction)*backend.sum(prediction**2) - backend.sum(prediction)**2
...
@eager_no_cache
def cv(sensitive: GraphSignal, prediction: GraphSignal, exclude: Optional[GraphSignal] = None) -> float:
    assert sensitive and prediction
    prediction = prediction.filter(exclude)
    sensitive = sensitive.filter(exclude)
    sensitive_rate = backend.sum(sensitive*prediction) / backend.sum(sensitive)
